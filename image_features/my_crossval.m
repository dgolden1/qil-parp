function [loss, predictedValues, coefficients, cvparray] = my_crossval(funorStr,varargin)
%CROSSVAL cross-validation
%   Modified by Dan to also return predicted function values
%   $Id: my_crossval.m 137 2013-01-08 00:03:38Z dgolden $
%   
% 
%   [VALS, PREDICTED_VALS, CVP_ARRAY] = CROSSVAL(FUN,X) performs 10-fold cross-validation for the
...
% 
%   PREDICTED_VALS is a length-P cell array of NxM matrices, representing P
%   monte carlo repetitions, N CV partitions and M values per partition
%
...
% $Revision: 1.1.8.8 $  $Date: 2011/10/15 02:06:15 $
...
if strcmp(firstInputType,'func') % the first input is a function handle
    ...
    [funResult, thisFunValPred, thisFunValCoeff] = getFuncVal(1, nData, cvp, data, funorStr, []);
    szFunResult = size(funResult);
    loss(numiter,:) = funResult(:)';
    loss(1,:) = funResult(:)';
    predictedValues(1, :) = thisFunValPred;
    coefficients(1, :) = thisFunValCoeff;
    ...
    if useParallel
        parfor j=2:numiter
            ...
            cvpj = cvparray{mcrep};
            [funResult, thisFunValPred, thisFunValCoeff] = getFuncVal(i, nData, cvpj, data, funorStr, szFunResult);
            
            % Accumulate the results
            loss(j,:) = funResult(:)';
            predictedValues(j, :) = thisFunValPred;
            coefficients(j, :) = thisFunValCoeff;
        ...
        for mr = 1:mcreps
            ...
            for i = loopstart:cvp.NumTestSets
                [funResult, thisFunValPred, thisFunValCoeff] = getFuncVal(i, nData, cvp, data, funorStr, szFunResult);
                loss(offset+i,:) = funResult(:)';
                predictedValues(offset+i, :) = thisFunValPred;
                coefficients(offset+i, :) = thisFunValCoeff;
            end
        end  % for mr=1:mcreps
    end
...
try
    [funResult{1}, funResult{2}, funResult{3}] = fun(arg{:});
catch ME
...
function [funResult, funValPred, funValCoeff] = getFuncVal(i, nData, cvp, data, funorStr, szFunResult)
arg = cell(2*nData,1);
...
funResultPlus = evalFun(funorStr,arg(:));
funResult = funResultPlus{1}; % The cross-validation criteria
funValPred = funResultPlus{2}; % The values predicted from the model
funValCoeff = funResultPlus{3}; % The model coefficients

if ~iscell(funValPred)
  % funValPred must be a cell to be properly accumulated in the
  % predictedValues variable by the caller
  % This is true by default for lasso, but not other cross-validation
  % functions
  funValPred = {funValPred};
end
