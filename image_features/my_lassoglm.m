function [B,stats] = my_lassoglm(x,y,distr,varargin)
%LASSOGLM Perform lasso or elastic net regularization for a generalized linear model.
% 
%   Modified by Dan to return predicted values
%   $Id: my_lassoglm.m 137 2013-01-08 00:03:38Z dgolden $
% 
...
%   $Revision: 1.1.6.1.4.1 $  $Date: 2012/01/06 20:32:55 $
...
[~,sqrtvarFun,devFun,linkFun,dlinkFun,ilinkFun,link,mu,eta,muLims] = ...
    glmProcessDistrAndLink(Y,distr,link,'off',nTrials,dataClass);

[X,Y,pwts,nTrials,lambda,dfmax,cvp,mcreps,lambdaMax,predictorNames,ever_active,nullDev] = ...
    processLassoParameters(X,Y,distr,link,pwts,offset,nTrials,alpha,nLambda,lambdaRatio,lambda,dfmax, ...
...
penalizedFitPartition = @(x,y,offset,pwts,n,wlsfit,b,active,mu,eta,sqrtvarFun) ...
    ...
    sqrtvarFun,linkFun,dlinkFun,ilinkFun,devFun,b,active,mu,eta,muLims,wlsfit,nullDev,reltol);
...
[B,Intercept,lambda,deviance] = ...
    lassoFit(X,Y,pwts,lambda,alpha,dfmax,standardize,reltol,lambdaMax,ever_active, ...
    penalizedFit,mu,eta,dataClass,userSuppliedLambda,nullDev);
...
if ~isequal(cvp,'resubstitution')
    ...
    cvfun = @(Xtrain,Ytrain,Xtest,Ytest) lassoFitAndPredict( ...
        ...
        lambda,alpha,P,standardize,reltol,lambdaMax,ever_active, ...
    ...
    [cvDeviance, predictedValues, cv_coefficients, cvparray] = my_crossval(cvfun,[weights(:) offset(:) X],response, ...
        'Partition',cvp,'Mcreps',mcreps,'Options',parallelOptions);
    % Scale single-fold deviances up to estimate whole data set deviance
    % cvDeviance = diag(size(X,1) ./ repmat(cvp.TestSize, 1, mcreps)) * cvDeviance; % repmat command added to fix error --DIG 2012-08-10 
...
% predictedValues is a cell array with num rows equal to num CV partitions
% times num MC reps, and num columns equal to num lambdas
% First, rearrange it to order by ascending Lambda, as above
predictedValues = predictedValues(:,reverseIndices);
cv_coefficients = cv_coefficients(:,reverseIndices);

% Then repack it into a more sensible structure
predictedValuesOld = predictedValues; % For debugging
...
stats.predictedValues = predictedValues;
stats.CVcoefficients = cv_coefficients;
...
function [X,Y,weights,nTrials,lambda,dfmax,cvp,mcreps,lambdaMax,predictorNames,ever_active,nullDev] = ...
    processLassoParameters(X,Y,distr, link, weights, offset, nTrials,alpha, nLambda, lambdaRatio, lambda, dfmax, ...
        standardize, reltol, cvp, mcreps, predictorNames)
...
if ~isempty(weights)
    ...
    % Normalize weights are used for standardization and calculating lambdaMax.
...
% In this function, standardization of the input data is only considered
% for the purpose of calculating lammbdaMax.  The function does not return
% X or Y in standardized form.
...
function [b,mu,eta,varargout] = glmIRLS(x,y,distr,offset,pwts,dataClass,N, ...
    sqrtvarFun,dlinkFun,ilinkFun,b,active,mu,eta,muLims, ...
...
function [B,active,varargout] = glmIRLSwrapper(X,Y,distr,offset,pwts,dataClass,N, ...
    sqrtvarFun,~,dlinkFun,ilinkFun,devFun,b,active,mu,eta,muLims, ...
...
function [dev, mu, BplusCell] = lassoFitAndPredict(Xtrain,Ytrain,Xtest,Ytest, ...
...
% current fold.
% 
% mu is the output of the lasso model for the test data
...
[B,Intercept] = lassoFit(Xtrain,Ytrain, ...
    ...
    lambdaMax,ever_active,penalizedFit,mu,eta,dataClass,true,[]);
Bplus = [Intercept; B];
...
for i=1:numFits
    if ~isequal(testOffset,0)
        mu{i} = glmval(Bplus(:,i), Xtest, link, 'Offset',testOffset);
    else
        mu{i} = glmval(Bplus(:,i), Xtest, link);
    end
    di = devFun(mu{i},Ytest);
    dev(i) = sum(testWeights' * di);
end

% Turn Bplus into a cell array, once cell per lambda
BplusCell = mat2cell(Bplus, size(Bplus, 1), ones(1, size(Bplus, 2)));
...
function [B,Intercept,lambda,varargout] = ...
    ...
    ~,ever_active,penalizedFit,mu,eta,dataClass,userSuppliedLambda,nullDev)
%
% ------------------------------------------------------
...
for i = 1:nLambda
    ...
    if i==1 && sum(active)~=0 && ~userSuppliedLambda
        % Increase lambdas if the greatest lambda fails to give all zero
        % coefficients. 
        nbumps = 0;
        if nLambda > 1 
            while sum(active)~=0 && nbumps < nLambda
                nbumps = nbumps + 1;
                b = zeros(P,1,dataClass);
                lambda = lambda * (lam / lambda(2));
                lam = lambda(1);
                wlsfit = @(x,y,weights,b,active) glmPenalizedWlsWrapper(y,x,b,active,weights,lam, ...
                    alpha,reltol,ever_active);
                [b,active,extras,mu,eta] = penalizedFit(X0,Y0,wlsfit,[intercept;b],active,mu,eta);
    ...
    if i==2 && sum(active)==0 && ~userSuppliedLambda
        % Prune extra lambdas that generate all-zero coefficients
        while sum(active)==0
            B(:,1) = b;
            Extras(:,1) = extras;
            % Redefine lambda sequence to discard excessive penalties
            lambda = lambda * (lam / lambda(1));
            lam = lambda(2);
            wlsfit = @(x,y,weights,b,active) glmPenalizedWlsWrapper(y,x,b,active,weights,lam, ...
                alpha,reltol,ever_active);
            [b,active,extras,mu,eta] = penalizedFit(X0,Y0,wlsfit,[intercept;b],active,mu,eta);            
