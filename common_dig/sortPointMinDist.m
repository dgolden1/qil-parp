function [xDistSort,yDistSort,minDist]=sortPointMinDist(x,y)
%% function [xDistSort,yDistSort,minDist]=sortPointMinDist(x,y)
...
[mindist(1),pointLinInd] = min(distMat(:));
[iDistSort(1),nextPoint] = ind2sub(size(distMat), pointLinInd);
for iPoint=1:nPoints-1
   currPoint=nextPoint;
   [minDist(iPoint+1),nextPoint] = min(distMat(currPoint, :)); % find next point- closest to current
   ...
   if isinf(minDist(iPoint+1)) % this will b true in case of an error
      break;
   end
end
