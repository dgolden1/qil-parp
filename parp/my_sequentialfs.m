function [in,history] = my_sequentialfs(fun,varargin)
%SEQUENTIALFS Sequential feature selection.
%   Modified by Dan to allow 'CV' to be equal to 1 $Id$
%
...
switch direction
    case 'forward'
        ...
        if  ~isempty(find(in,1)) || nullmodel 
            ...
            x = X(:,in);
            [initCrit, initCritSE] = callfun(fun,x,other_data,cv,mcreps,ParOptions);
            history.In = [history.In; in];
            history.Crit = [history.Crit, initCrit];
            history.CritSE = [history.CritSE, initCritSE];
        end
        ...
        if options.Display > 1
            ...
            if ~isempty(find(in,1)) || nullmodel
                disp(sprintf('Step 1, used initial columns, criterion value %g +/- %g',...
                      history.Crit(end), history.CritSE(end)/2));
            end
        ...
        for j = 1:nStart
            ...
            critSE = zeros(1,numAvailable);

            for k = 1:numAvailable
                x(:,end) = X(:,available(k));
                [crit(k), critSE(k)] = callfun(fun,x,other_data,cv,mcreps,ParOptions);
            end
            [bestCrit,idx] = min(crit);  % minimize the cost.
            bestCritSE = critSE(idx);
            ...
            history.CritSE = [history.CritSE, bestCritSE];
            if options.Display > 2 %iter
                txt=sprintf('Step %d, added column %d, criterion value %g +/- %g',...
                    length(history.Crit),nextOne,bestCrit,bestCritSE/2);
                disp(txt);
            end            
        end
...
%-----------------------------------
function [crit, critSE] = callfun(fun,x,other_data,cv,mcreps,ParOptions)
if isa(cv,'cvpartition')
    ...
    funResultNorm = funResult./ (mcreps * cv.TestSize.');
    critSE = std(funResultNorm)/sqrt(sum(cv.TestSize));
else
    try
